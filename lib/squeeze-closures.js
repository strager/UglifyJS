...
function ast_squeeze_closures(ast) {
        ...
        function split_vars(ast) {
                ...
                return w.with_walkers({
                        ...
                        "toplevel": function(stats) {
                                return [ this[0], statements(stats) ];
                        },
        ...
        function inline(oldName, value, ast) {
                var w = pro.ast_walker(), walk = w.walk;

                return w.with_walkers({
                        "name": function(name) {
                                if (name == oldName) return value;
                        }
                }, function() {
                        return walk(ast);
                });
        ...
        return w.with_walkers({
                ...
                "function": function(name, args, body) {
                        body = map_statements(body, walk);
                        body = map_statements(body, split_vars);
                        body = map_statements(body, scope.ast_make_names);

                        var nbody = [];
                        MAP(body, function(stat) {
                                var fcallPattern = [ "call", [ "function", null, one("fnargs"), many("body", [
                                        [ "return", one("retval") ],
                                        one("vardef", [ "var", _ ]),
                                        one("statement", _)
                                ]) ], many("callargs", [ [ "name", _ ] ]) ];

                                var fcallMatch = ast_match(stat, fcallPattern);
                                var statMatch = ast_match(stat, [ "stat", fcallPattern ]);
                                ...
                                var match = fcallMatch || statMatch || assignMatch || varMatch;

                                if (match && can_expand_body(match.body)) {
                                        function inline_arguments(ast) {
                                                var callargs = match.callargs;
                                                MAP(match.fnargs, function(fnarg, i) {
                                                        if (callargs[i]) ast = inline(fnarg, callargs[i], ast);
                                                });
                                                return ast;
                                        }
                                        ...
                                        MAP(match.body, function(node) {
                                                ...
                                                if (node.retval) {
                                                        returned = inline_arguments(node.retval);
                                                } else if (node.vardef) {
                                                        nbody.push([ "var", MAP(node.vardef[1], function(def) {
                                                                return [ rename_name(def[0]), inline_arguments(def[1]) ];
                                                        }) ]);
                                                } else {
                                                        nbody.push(inline_arguments(node.statement));
                                                }
                                        });
